...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#0B130E] flex">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-[260px] bg-[#0A120D] border-r border-emerald-500/15 flex flex-col transition-transform duration-300 ${
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                  isActive
                    ...
                    : 'text-slate-400 hover:text-slate-200 hover:bg-[#111E16] border border-transparent'
                }`}
              >
                <Icon size={16} />
                {item.label}
